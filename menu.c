...
struct menu *
menu_create (menu_callback callback)
{
...
void
menu_getopts (struct menu *menu, int argc, char *argv[])
{
  const char *usage
      ...
      switch (opt)
        ...
        case 'S':
          if (!parse_color (optarg, &menu->selectionbg))
            {
              fprintf (stderr, "Invalid selection background color: %s",
                       optarg);
            }
          break;
        case 's':
          if (!parse_color (optarg, &menu->selectionfg))
            {
              fprintf (stderr, "Invalid selection foreground color: %s",
                       optarg);
            }
          break;
        case 'w':
  ...
  /* apply popup colors for center mode */
  if (menu->position == POSITION_CENTER)
    {
      menu->normalbg = popup_normalbg;
      menu->normalfg = popup_normalfg;
      menu->selectionbg = popup_selbg;
      menu->selectionfg = popup_selfg;
      menu->promptbg = popup_promptbg;
      menu->promptfg = popup_promptfg;
    }
  ...
  if (menu->lines > 0)
    {
      int actual_lines
          = (menu->item_count > 0 && menu->item_count < (size_t)menu->lines)
                ? (int)menu->item_count
                : menu->lines;
      menu->height += menu->height * actual_lines;
    }
...
void
menu_sort_and_deduplicate (struct menu *menu)
{
  size_t j = 1;
  size_t i;
...
static void
append_page (struct page *page, struct page **first, struct page **last)
{
  if (*last)
    {
      (*last)->next = page;
...
static const char *
fstrstr (struct menu *menu, const char *s, const char *sub)
{
...
static void
append_match (struct item *item, struct item **first, struct item **last)
{
  if (*last)
    {
...
void
menu_invalidate (struct menu *menu)
{
  menu->rendered = false;
}
...
static void
insert (struct menu *menu, const char *text, ssize_t len)
{
...
void
menu_paste (struct menu *menu, const char *text, ssize_t len)
{
  insert (menu, text, len);
}

static size_t
nextrune (struct menu *menu, int incr)
{
  size_t n, len;

  len = strlen (menu->input);
  for (n = menu->cursor + incr; n < len && (menu->input[n] & 0xc0) == 0x80;
       n += incr)
    ;
  return n;
}

// Move the cursor to the beginning or end of the word, skipping over any
// preceding whitespace.
static void
movewordedge (struct menu *menu, int dir)
{
  if (dir < 0)
    {
...
// Handle a keypress.
void
menu_keypress (struct menu *menu, enum wl_keyboard_key_state key_state,
               xkb_keysym_t sym)
{
