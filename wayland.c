...
output_create (struct wl_context *context, struct wl_output *wl_output)
{
...
keyboard_create (struct menu *menu, struct wl_keyboard *wl_keyboard)
{
...
// Frees the keyboard.
static void
free_keyboard (struct keyboard *keyboard)
{
...
int
context_get_scale (struct wl_context *context)
{
...
struct pool_buffer *
context_get_current_buffer (struct wl_context *context)
{
...
struct pool_buffer *
context_get_next_buffer (struct wl_context *context, int scale)
{
  struct menu *menu = context->menu;
  context->current = get_next_buffer (context->shm, context->buffers,
                                      menu->width, menu->height, scale);
  return context->current;
}

// Returns the Wayland surface for the context.
struct wl_surface *
context_get_surface (struct wl_context *context)
{
...
struct xkb_state *
context_get_xkb_state (struct wl_context *context)
{
...
context_get_xdg_activation (struct wl_context *context)
{
...
bool
context_paste (struct wl_context *context)
{
...
static void
context_add_output (struct wl_context *context, struct output *output)
{
...
// Frees the outputs.
static void
free_outputs (struct wl_context *context)
{
...
static void
context_destroy (struct wl_context *context)
{
...
static void
surface_enter (void *data, struct wl_surface *surface,
...
output_name (void *data, struct wl_output *wl_output, const char *name)
{
  ...
  if (context->menu->output_name
      && strcmp (context->menu->output_name, name) == 0)
    {
                 ...
                 int32_t fd, uint32_t size)
{
  ...
  keyboard->keymap = xkb_keymap_new_from_string (keyboard->context, map_shm,
...
static void
keyboard_repeat (struct keyboard *keyboard)
{
  menu_keypress (keyboard->menu, keyboard->repeat_key_state,
                 keyboard->repeat_sym);
               ...
               const char *interface, uint32_t version)
{
  ...
  if (strcmp (interface, wl_compositor_interface.name) == 0)
    ...
    {
      context->data_device_manager = wl_registry_bind (
          registry, name, &wl_data_device_manager_interface, 3);
    }
  else if (strcmp (interface, zwlr_layer_shell_v1_interface.name) == 0)
    {
      context->layer_shell = wl_registry_bind (
...
menu_run (struct menu *menu)
{
  ...
  struct zwlr_layer_surface_v1 *layer_surface
      = zwlr_layer_shell_v1_get_layer_surface (
          context->layer_shell, context->surface,
